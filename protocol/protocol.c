 ...
 */
int8_t receiving_process(Pid_System_t* pid_system)
{
  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        pid_system->Set_P_I_D(pid_system,p_temp, i_temp, d_temp);    // 设置 P I D
      }
      break;
...
//        pid_system->pid->user_target_val= (double)(actual_temp)/1000;       // 设置目标值
          pid_system->pid->target_val= (double)(actual_temp)/1000;    
      ...
      case START_CMD:
      {    
        if(pid_system->pid->ID ==1){
            
//            HAL_TIM_Base_Start_IT(&htim2);            //开启TIM2
//          printf("开启恒流PID模式\n");
//            
          }
       else if(pid_system->pid->ID ==2){
...
void set_computer_value(UART_HandleTypeDef* huart, uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  uint8_t sum = 0;    // 校验和
  num *= 4;           // 一个参数 4 个字节
  ...
  HAL_UART_Transmit(huart, (uint8_t *)&set_packet, sizeof(set_packet), 0xFFFFF);    // 发送数据头
  HAL_UART_Transmit(huart, (uint8_t *)data, num, 0xFFFFF);                          // 发送参数
  HAL_UART_Transmit(huart, (uint8_t *)&sum,sizeof(sum), 0xFFFFF);                  // 发送校验和
}
